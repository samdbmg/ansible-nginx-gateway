...
(
    ...
    openssl genrsa -out fake-client-ca.key 2048
    openssl req -x509 -new -nodes -key fake-client-ca.key -sha256 -days 2048 -subj "/C=GB/ST=Manchester/O=Sample/CN=fake-client-cert." -out fake-client-ca.crt

    # Generate key for cert
    openssl genrsa -out fake-client-cert.key 2048
